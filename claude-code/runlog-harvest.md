...
| **Session-context discovery** | "In-frame fallback; per-host transcript optional" | Claude Code stores transcripts at `~/.claude/projects/<encoded-cwd>/<session-id>.jsonl` — the adapter MAY read this for richer signal, but the in-frame conversation is the normative fallback and works on every machine. |
