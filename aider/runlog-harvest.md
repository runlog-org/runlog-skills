...
| **Session-context discovery** | "In-frame fallback; per-host transcript optional" | Aider keeps `.aider.chat.history.md` in the working directory. Per harvest contract OQ #3 the canonical body falls back to in-frame conversation context; the chat-history file is an OPTIONAL convenience for users who want explicit transcript scanning (`/run cat .aider.chat.history.md` to feed it into the agent). The default and normative path is in-frame. |
