...
| **Session-transcript discovery** | "In-frame fallback; per-host transcript optional" | Falls back to in-frame conversation context. Recent-commits scan via the IDE's built-in terminal driven by AI Assistant on supported versions. No JetBrains-specific transcript file is consulted. |
