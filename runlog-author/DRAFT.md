...
- **Where does the skill live in `skills/`?** Probably `skills/runlog-author/SKILL.md` parallel to `claude-code/SKILL.md`, or nested under `claude-code/author/SKILL.md`. The latter signals "this is part of the Claude Code skill family"; the former signals "this is portable across MCP clients." Lean: parallel, since the read-side `runlog` skill is portable too.
